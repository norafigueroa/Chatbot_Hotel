...
interface FloatingWidgetsProps {
  ...
  onToggleChat: () => void
  /**
   * true cuando corre como widget embebido en el sitio real: se omiten el
   * Privacy y el WhatsApp decorativos de la demo (el sitio ya tiene los suyos)
   * y el lanzador del chat se ve más grande y con fondo de marca.
   */
  embedded?: boolean
}
...
export default function FloatingWidgets({
  ...
  return (
    <div
      className={`pointer-events-none fixed bottom-0 left-0 right-0 flex items-end p-4 ${
    ...
    >
      ...
      <div className="pointer-events-auto flex flex-col items-center gap-3">
        ...
        <button
          ...
          className={`flex items-center justify-center rounded-full shadow-lg transition-transform duration-300 hover:scale-110 ${
            embedded
              ? 'h-[72px] w-[72px] bg-brand-teal shadow-2xl ring-4 ring-white'
              : `h-16 w-16 ${chatOpen ? 'bg-brand-teal' : ''}`
          }`}
        >
          {chatOpen ? (
            <svg
            ...
            <img
              ...
              alt=""
              aria-hidden="true"
              className={`animate-pulse-glow object-contain drop-shadow-lg ${
        ...
        {/* WhatsApp: solo visual de la demo; no se muestra en el widget embebido. */}
        {!embedded && (
